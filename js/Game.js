    ...
    play(){
      ...
      if(allPlayers !== undefined){
        ...
        for(var plr in allPlayers){
          ...
          if (frameCount % 35 === 0) {
           var carrots = createSprite(random(50,displayHeight*4),random(50,displayHeight*4), 100, 100);
      ...
      console.log("end!")
        ...
        for(var plr in allPlayers){
          ...
          if(allPlayers[plr].rank===1){
            fill("blue");
            text("First place goes to: "+allPlayers[plr].name,displayWidth/2,200+"Score is: "+allPlayers[plr].score,displayWidth/2+80,200);
          }
          if(allPlayers[plr].rank===2){
            fill("green");
            text("Second place goes to: "+allPlayers[plr].name,displayWidth/2,270+"Score is: "+allPlayers[plr].score,displayWidth/2+80,200);
          }
          if(allPlayers[plr].rank===3){
            fill("yellow")
            text("Third place goes to: "+allPlayers[plr].name,displayWidth/2,340+"Score is: "+allPlayers[plr].score,displayWidth/2+80,200);
          }
          if(allPlayers[plr].rank===4){
            fill("red")
            text("The loser is: "+allPlayers[plr].name,displayWidth/2,410+"Score is: "+allPlayers[plr].score,displayWidth/2+80,200);
          }
        }
      }
  }
